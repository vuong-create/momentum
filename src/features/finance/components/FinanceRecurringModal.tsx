...
} from "../services/financeRecurringService";
...
export default function FinanceRecurringModal({ accounts, categories, items, todayKey, onClose, onCreate, onUpdate, onToggle, onDelete, onConfirm, onSkip }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    await act(async () => {
      const nextInput = { ...input, amount: Number(amountText) };
      if (editingId) await onUpdate(editingId, nextInput);
      else await onCreate(nextInput);
      setEditorOpen(false);
      setEditingId(null);
    });
  }
  ...
  function changeType(type: RecurringType) {
    setInput((current) => ({
      ...current,
      type,
      accountId: undefined,
  ...
  function changeFrequency(frequency: FinanceRecurringInput["frequency"]) {
    setInput((current) => ({
      ...current,
      frequency,
      customInterval: frequency === "custom"
        ? current.customInterval ?? 1
        : current.customInterval,
      customUnit: frequency === "custom"
        ? current.customUnit ?? "months"
        : current.customUnit,
    }));
  }
  ...
  return <div className="finance-modal-layer" role="dialog" aria-modal="true" aria-labelledby="finance-recurring-title"><button className="finance-modal-backdrop" type="button" aria-label="Close" onClick={onClose} /><section className="finance-modal finance-recurring-modal"><header><div><span className="text-label">Review before posting</span><h2 id="finance-recurring-title">Recurring transactions</h2></div><button type="button" onClick={onClose}>×</button></header><div className="finance-recurring-body">
    ...
    {editorOpen && <form className="finance-recurring-editor" onSubmit={submit}><header><strong>{editingId ? "Edit schedule" : "New recurring item"}</strong><button type="button" aria-label="Close recurring editor" onClick={() => setEditorOpen(false)}>×</button></header><div className="finance-type-switch">{financeTransactionTypes.map(({ value, label }) => <button type="button" key={value} className={input.type === value ? "is-selected" : ""} onClick={() => changeType(value)}>{label}</button>)}</div><div className="finance-modal-fields">
      ...
      <label><span>Amount</span><div className="finance-money-input"><i>$</i><input inputMode="decimal" value={amountText} onChange={(event) => changeAmount(event.target.value)} placeholder="0.00" aria-label="Recurring transaction amount" /></div></label>
      ...
      <label><span>Frequency</span><select value={input.frequency} onChange={(event) => changeFrequency(event.target.value as FinanceRecurringInput["frequency"])}><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="yearly">Yearly</option><option value="custom">Custom…</option></select></label>
      {input.frequency === "custom" && <label><span>Repeat every</span><div className="finance-recurring-interval"><input type="number" min="1" step="1" value={input.customInterval ?? 1} onChange={(event) => patch("customInterval", Math.max(1, Math.floor(Number(event.target.value) || 1)))} aria-label="Custom repeat interval" /><select value={input.customUnit ?? "months"} onChange={(event) => patch("customUnit", event.target.value as FinanceRecurrenceUnit)} aria-label="Custom repeat unit"><option value="days">Days</option><option value="weeks">Weeks</option><option value="months">Months</option><option value="years">Years</option></select></div></label>}
