...
export default function FinanceOverview({ accounts, transactions, categories: categoryRecords, now, balancesHidden, onToggleBalances, onAddAccount, onOpenTransactions }: Props) {
  ...
  return <div className="finance-overview">
    ...
    <div className="finance-overview-grid">
      ...
      <section className="finance-panel finance-category-summary"><header><div><span className="text-label">Spending</span><h3>By category</h3></div><small>{new Intl.DateTimeFormat("en-US", { month: "long" }).format(now)}</small></header>{categories.length ? <div>{categories.slice(0, 5).map((item, index) => <article key={item.category}><span className="finance-category-rank">{String(index + 1).padStart(2, "0")}</span><div className="finance-category-body"><header><span>{item.category}</span><strong className="finance-balance-value">{formatMoney(item.amount)}</strong></header><i><span style={{ width: `${item.amount / categories[0].amount * 100}%` }} /></i><small>{Math.round(item.amount / categoryTotal * 100)}% of categorized spending</small></div></article>)}</div> : <p className="finance-empty-copy">Expenses will organize themselves here.</p>}</section>
    </div>
