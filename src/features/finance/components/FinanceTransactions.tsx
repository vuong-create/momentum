...
interface Props { accounts: FinanceAccount[]; categories: FinanceCategory[]; transactions: FinanceTransaction[]; onEdit: (item: FinanceTransaction) => void; onDelete: (item: FinanceTransaction) => Promise<void>; }

export default function FinanceTransactions({ accounts, categories, transactions, onEdit, onDelete }: Props) {
  ...
  return <section className="finance-transactions-view">
    ...
    <div className="finance-transaction-table finance-panel"><header><span>Date</span><span>Merchant</span><span>Category</span><span>Account</span><span>Amount</span><span /></header>{filtered.length ? filtered.map((item) => {
      ...
      const category = categories.find((entry) => entry.id === item.categoryId)?.name ?? item.category;
      return <article key={item.id} onDoubleClick={() => onEdit(item)}><time>{new Date(`${item.date}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</time><div><span className={`finance-transaction-glyph type-${item.type}`}>{item.type === "income" ? "＋" : item.type === "transfer" ? "↔" : item.type === "investment" ? "↗" : "−"}</span><strong>{item.merchant}</strong></div><span>{category ?? (item.type === "transfer" ? "Transfer" : item.type)}</span><span>{account}</span><b className={signed > 0 ? "is-positive" : signed < 0 ? "is-negative" : ""}>{item.type === "transfer" ? formatMoney(item.amount) : `${signed > 0 ? "+" : ""}${formatMoney(signed)}`}</b><menu><button type="button" onClick={() => onEdit(item)}>Edit</button><button type="button" onClick={() => onDelete(item)}>Delete</button></menu></article>;
