...
const cardTones = ["saffron", "miso", "sage", "charcoal", "plum"];
      ...
      <div className="cooking-recipe-grid">
        {filtered.map((recipe, index) => (
          <article key={recipe.id} className="cooking-recipe-card">
            <button type="button" className={`cooking-recipe-cover cooking-tone-${cardTones[index % cardTones.length]}`} onClick={() => setSelected(recipe)}>
              <span>{recipe.tags[0] ?? "Home cooking"}</span><strong>{recipe.name.charAt(0)}</strong><i aria-hidden="true">湯</i>
            </button>
            <div>
