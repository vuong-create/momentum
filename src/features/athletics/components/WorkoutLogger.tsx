    ...
    <section className="athletics-workout-view">
      ...
      <div className="athletics-exercise-stack">
        {workout.exercises.map((exercise, exerciseIndex) => (
          <article className="athletics-exercise-card" key={exercise.id}>
            <header>
              <span>{String(exerciseIndex + 1).padStart(2, "0")}</span>
              <div><h3>{exercise.name}</h3><small>{exercise.sets.filter((set) => set.completed).length} of {exercise.sets.length} sets</small></div>
              <button type="button" onClick={() => onRemoveExercise(exercise.id)} aria-label={`Remove ${exercise.name}`}>Remove</button>
            </header>
            <div className="athletics-set-labels"><span>Set</span><span>Weight</span><span>Reps</span><span>Quick adjust</span><span>Done</span></div>
            <div className="athletics-set-list">
              {exercise.sets.map((set, setIndex) => (
                <div className={`athletics-set-row${set.completed ? " is-complete" : ""}`} key={set.id}>
                  <span className="athletics-set-number">{setIndex + 1}</span>
                  <label><input type="number" inputMode="decimal" min="0" value={set.weight} onChange={(event) => onUpdateSet(exercise.id, set.id, { weight: Number(event.target.value) })} /><small>lb</small></label>
