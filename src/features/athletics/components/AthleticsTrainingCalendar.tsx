...
type Props = {
  ...
  onStart: (session: AthleticsPlannedSession) => Promise<void>;
  onLogVolleyball: (session: AthleticsPlannedSession) => Promise<void>;
...
export default function AthleticsTrainingCalendar({ block, sessions, activities, now, selectedSessionId, installing, onInstall, onSelect, onStart, onLogVolleyball, onSaturdayChoice, onMove, onSkip, onExerciseChoice }: Props) {
  ...
  return <section className="athletics-training-calendar">
    ...
    {selected && <div className="athletics-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onSelect(null); }}><section className="athletics-session-modal"><header><div><span className="text-label">Week {selected.weekNumber} · {selected.phaseName}</span><h2>{selected.name}</h2><p>{formatDate(getSessionScheduledDate(selected, activities), { weekday: "long", month: "long", day: "numeric" })} · {selected.focus}</p></div><button type="button" onClick={() => onSelect(null)}>×</button></header>
      ...
      {selected.exercises.length > 0 && <div className="athletics-session-exercises">{(["explosive", "hypertrophy"] as const).map((category) => {
        ...
        return <section key={category}><header><span>{category === "explosive" ? "Athletic / Explosive" : "Strength / Hypertrophy"}</span><small>{category === "explosive" ? "Quality first · full recovery" : "Double progression"}</small></header>{exercises.map((exercise, index) => <article key={exercise.id}><span>{String(index + 1).padStart(2, "0")}</span><div>{exercise.alternatives?.length ? <select value={exercise.name} onChange={(event) => onExerciseChoice(selected.id!, exercise.id, event.target.value)}>{exercise.alternatives.map((option) => <option key={option}>{option}</option>)}</select> : <strong>{exercise.name}</strong>}<small>{exercise.category === "explosive" ? "Complete for quality" : "Track load + reps"}</small></div><b>{exercise.targetLabel}</b></article>)}</section>;
      })}</div>}
      {selected.notes && <p className="athletics-session-notes">{selected.notes}</p>}
      <footer><div>{selectedStatus === "missed" && <><strong>Missed · choose what happens next</strong><button type="button" onClick={() => onMove(selected, todayKey)}>Move to today</button></>}<label>Choose date<input type="date" value={moveDate} onChange={(event) => setMoveDate(event.target.value)} /></label>{moveDate && <button type="button" onClick={async () => { await onMove(selected, moveDate); setMoveDate(""); }}>Move</button>}<button type="button" className="is-skip" disabled={selected.kind === "recovery" || selectedStatus === "completed" || selectedStatus === "skipped"} onClick={() => onSkip(selected)}>Skip</button></div>{selected.kind === "gym" && selectedStatus !== "skipped" && selectedStatus !== "completed" && <button type="button" className="athletics-primary-button" onClick={() => onStart(selected)}>Start workout →</button>}{selected.kind === "volleyball" && selectedStatus !== "skipped" && selectedStatus !== "completed" && <button type="button" className="athletics-primary-button" onClick={() => onLogVolleyball(selected)}>Log volleyball ✓</button>}</footer>
    </section></div>}
  </section>;
}
